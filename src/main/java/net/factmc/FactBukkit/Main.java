...
public class Main extends JavaPlugin implements Listener {
	...
	public void onEnable() {
		 ...
		 * 
		 * roles.clear(); roles.add(Rel.RECRUIT); roles.add(Rel.MEMBER);
		 * roles.add(Rel.OFFICER); roles.add(Rel.LEADER); }
		 */
		 ...
		 * getServer().getServicesManager().getRegistration(Permission.class); perms =
		 * permRSP.getProvider(); plugin.getLogger().info("Connected to " +
		 * perms.getName());
		 */
		...
		if (getConfig().getBoolean("clearlag.enabled")) {
			...
			String warningMessage = ChatColor.translateAlternateColorCodes('&',
					getConfig().getString("clearlag.warning-message"));
			String clearMessage = ChatColor.translateAlternateColorCodes('&',
					getConfig().getString("clearlag.clear-message"));
	...
	public void registerEvents() {
		...
		if (!disableRankTag) {
			getServer().getPluginManager().registerEvents(new JoinEvents(), plugin);
			new LuckPermsEvents();
		}
	...
	public static void registerTeams(Scoreboard sb) {
		...
		/*
		 * if (useFactions) { for (Team team : sb.getTeams()) { if
		 * (team.getName().startsWith("f")) { team.unregister(); } }
		 * 
		...
		for (Group raw : LuckPermsProvider.get().getGroupManager().getLoadedGroups()) {
			...
			for (int i = 0; i < 2; i++) {
				...
				Team team;
				String teamName = "rank_" + group + add;
				try {
		...
		/*
		 * if (useVanish) { Team oldTeam = sb.getTeam("rank_vanished"); if (oldTeam !=
		 * null) oldTeam.unregister(); Team team = sb.registerNewTeam("rank_vanished");
		 * team.setSuffix(ChatColor.GRAY + "[HIDDEN]"); }
		 */
